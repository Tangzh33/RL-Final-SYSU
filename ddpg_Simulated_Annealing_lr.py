...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        x = self.linear3(x) 
        '''For simpilicity, no activation and range formalization for output layer, because we want to output the action directly'''
    ...
    def sample_action(self, action_range=1.0):
        '''select action for actioning, return .cpu'''
        normal = Normal(0, 1)
...
class DDPG():
    ...
    def update(self, batch_size, gamma=0.99, soft_tau=1e-2, target_update_delay=2, episode = 0,  step = 0):
        ...
        '''Simulated Annealing to adjust the learning rate'''
        global_step = episode * args.record_steps_limit + step
        global_step_limits = args.episodes_limit * args.record_steps_limit
        SA_frac = 1.0 - (global_step - 1.0) / global_step_limits
        args.lr = SA_frac * args.lr
        q_lr=args.lr
        policy_lr = args.lr
        # print("Current lr is",q_lr, policy_lr)
        ...
        self.q_optimizer = optim.Adam(self.qnet.parameters(), lr=q_lr)
        self.policy_optimizer = optim.Adam(self.policy_net.parameters(), lr=policy_lr)
        '''Finish adding Simulated Annealing adapting'''
        ...
        for episode in range (args.episodes_limit):
            ...
            for step in range(args.record_steps_limit):
                ...
                if len(replay_buffer) > args.batch_size:
                    '''Update the network'''
                    q_loss, policy_loss = alg.update(args.batch_size, args.gamma, args.tau, args.target_update_delay, episode, step)
